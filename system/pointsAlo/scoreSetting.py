...
class ScoreTool(object):
    ...
    def scoreUpdate(self, user_id, new_id, tag, refer_time, operation):
        ...
        try:
            ...
            if len(cur_new_info_list) == 0 or cur_new_info_list[len(cur_new_info_list) - 1][3] == None or \
                            cur_new_info_list[len(cur_new_info_list) - 1][3] == 0:
    ...
    def calAllTagAddScore(self, tag, mainTagAddScore, cur_new_tagDeep_list):
        ...
        for i in range(len(cur_new_tagDeep_list)):
            if i % 2 == 0:
                if cur_new_tagDeep_list[i] != tag:
                    tagAddScore[cur_new_tagDeep_list[i]] = mainTagAddScore * (
                    cur_new_tagDeep_list[i + 1] * 1.0 / mainTagDeep)
    ...
    def getUserNewData(self, user_id, tag):
        db = newsDb()
        sql = "select news_id,weight,is_comment,score from user_behavior where news_tag='" + tag + "' and user_id='" + user_id + "'"
        return db.select_table_three(sql)
    ...
    def getCurNewTagList(self, new_id):
        ...
        data = db.select_table_three(sql)
